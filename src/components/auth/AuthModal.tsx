...
import { getFirestore, doc, setDoc, collection, increment, getDoc } from "firebase/firestore";
...
const handleRegistrationSubmit = async (values: RegistrationFormData) => {
  ...
  const uniqueMembershipNumber = `MZIP${currentYear}${paddedIndex}`;
    ...
    await setDoc(doc(fireDataBase, "userIndex", "userNumber"), {lastUserIndex: increment(1)}, {merge: true});
   setIsLoading(false);
  } catch (error) {
      ...
      <DialogContent className="bg-white p-0 sm:max-w-[500px] overflow-hidden">
        ...
        >
          ...
          <TabsContent value="login" className="m-0">
            <LoginForm onSubmit={handleLoginSubmit} onForgotPassword={handleForgotPassword} isLoading={isLoading} />
          </TabsContent>
